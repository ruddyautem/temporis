...
const BrandMark = ({ badge }: BrandMarkProps) => {
  ...
  return (
    <div className='text-center space-y-2 sm:space-y-5 font-mono py-1 sm:py-2'>
      <div>
        <div className='relative inline-flex items-center gap-2 text-[9.5px] sm:text-[11px] uppercase tracking-[0.25em] text-emerald-400 font-semibold px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-none border border-emerald-500/30 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.15)]'>
      ...
      <div className='py-0.5 sm:py-2'>
        <h1 className='text-3xl sm:text-5xl md:text-6xl font-black tracking-tight text-white leading-none font-mono'>
          <span className='text-emerald-400'>{">"}</span>TEMPOR<span className='text-emerald-400'>IS</span>
        </h1>
      </div>

      <p className='text-[9.5px] sm:text-[11px] uppercase tracking-[0.22em] sm:tracking-[0.25em] text-slate-400 max-w-xs sm:max-w-md mx-auto leading-relaxed'>
        {t("subtitle")}
      </p>

      {badge && <div className='pt-1 sm:pt-2'>{badge}</div>}
    </div>
  );
};
